...
def r_squared(y, estimated):
    ...
    # TODO
    y_data = np.asarray(y)
    estimated_data = np.asarray(estimated)
    mean = y_data.mean()
    
    numerator = ((y_data - estimated_data)**2).sum()
    denominator = ((y_data - mean)**2).sum()
...
def evaluate_models_on_training(x, y, models):
    ...
    for model in models:
        ...
        predicted_y = predict_function(x)
        r2 = r_squared(y, predicted_y.tolist())
        plt.scatter(x, y, c='b')
